...
export class DomandeComponent implements OnInit  {
  ...
  selectedFilterCategory: number | null = null; // Filtro separato per la lista domande
  ...
  constructor(private fb: FormBuilder, private apiService: ApiService,private router: Router,private dialog:MatDialog, private cdr: ChangeDetectorRef) {
    this.codiceProd = 0;
  }
  log(i:any){
    //console.log(i);
  ...
  removeOption2(index: number,id:any): void {
    ...
    if (id!=undefined) {
      ...
      this.form2.get('opzioniRimosse')?.setValue(this.questionCreatedEdit[0].optionDeleted);
    }
  ...
  // Metodo nativo per il select principale
  onCategorySelectNative(event: any) {
    const selectedId = event.target.value;
    this.selectedCategory = selectedId ? parseInt(selectedId) : null;
    
    // Aggiorna anche il form control se necessario
    if (this.form) {
      this.form.patchValue({ categoryId: this.selectedCategory });
    }
  }

  // Funzione separata per il filtro delle domande esistenti
  onFilterCategorySelect(event: MatSelectChange) {
    this.selectedFilterCategory = event.value as number;
  }

  // Metodo nativo per il filtro categorie
  onFilterCategorySelectNative(event: any) {
    const selectedId = event.target.value;
    this.selectedFilterCategory = selectedId ? parseInt(selectedId) : null;
  }

  // Funzione per filtrare le domande in base alla categoria selezionata
  getFilteredQuestions(): any[] {
    if (!this.selectedFilterCategory) {
      return this.listaDomande;
    }
    return this.listaDomande.filter(domanda => 
      domanda.macro_product?.id === this.selectedFilterCategory
    );
  }

  // Funzione per ottenere il nome della categoria selezionata per il filtro
  getSelectedFilterCategoryName(): string {
    if (!this.selectedFilterCategory) {
      return '';
    }
    const categoria = this.categorie.find(cat => cat.id === this.selectedFilterCategory);
    return categoria ? categoria.descrizione : '';
  }

  // TrackBy function per ottimizzare il rendering delle categorie
  ...
  editQuestion(index: any,type:string): void {
    ...
    if (this.questionToEdit.categoryId) {
      ...
      if (this.questionToEdit.type === 'select') {
        ...
        optionsArray.clear();
        this.questionToEdit.options?.forEach((option: any) => optionsArray.push(this.fb.control(option)));
      ...
      this.form2.get('obbligatorio')?.setValue(this.questionCreatedEdit[0].obbligatorio==1 ? '1' : '0');
      ...
      this.questionCreatedEdit[0].options.forEach((option: any, i: number) => {
        // Aggiorna il FormArray con i nuovi valori delle opzioni
        optionsArray2.setControl(i, this.fb.group({
          id: [option.id],
  ...
  deletequestionExist(domanda:any,reparto:string){
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) { // L'utente ha confermato l'eliminazione
        this.apiService.deleteQuestion(domanda.id).subscribe((risposta: any) => {
  ...
  submitQuestions(): void {
    ...
    if (this.createdQuestions.length>0) {
      ...
      this.apiService.salvaDomande(this.createdQuestions).subscribe(response => {
      ...
      this.apiService.salvaDomande(this.questionCreatedEdit).subscribe(response => {
        // Gestisci la risposta dell'API
         //console.log(response);
        this.questionCreatedEdit = [];
        this.shouldRefresh = true; // Attiva l'aggiornamento dopo la navigazione
