...
class AuthController extends Controller
{
    ...
    public function aggiornaImmagine(Request $request){
        ...
        if ($request->hasFile('file')) {
            ...
            $storagePath = 'userImage/' . $idUser . '/immagineUser';
    ...
    public function cancellaFile(Request $request){
        ...
        if ($storageFiles) {
            foreach ($storageFiles as $singleFile) {
                // Extract file name from path
                $nomeFile = explode("/", $singleFile);
                $splitNomeFile = explode("_", $nomeFile[1]);

                if ($splitNomeFile[2] == $request->fileName) {
                    Storage::delete($singleFile);
                    ...
                    $file = "File eliminato con successo " . $request->fileName;
                } else {
                    $file = "";
                }
            }
